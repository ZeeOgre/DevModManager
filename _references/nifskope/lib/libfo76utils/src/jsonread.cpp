...
std::string& JSONReader::readToken()
{
  ...
  while (filePos < fileBufSize) [[likely]]
  {
    ...
    if ((unsigned char) c <= 0x20)
    {
      if (!c)
      {
        throw NifSkopeError("invalid character in JSON file at line %lu",
      ...
      if (readingQuotedString)
      {
        if (c == '\n')
        {
          throw NifSkopeError("unexpected end of line %lu in JSON file",
                              (unsigned long) curLine);
        }
        else if (!(c == '\t' || c == ' '))
        {
    ...
    else if (c == '"')
    {
      if (!readingQuotedString)
      {
        throw NifSkopeError("syntax error in JSON file at line %lu",
                            (unsigned long) curLine);
      }
      curToken += c;
      return curToken;
        ...
        case 'u':
          {
            ...
            for (int i = 0; i < 4; i++)
            {
              ...
              {
                throw NifSkopeError("unexpected end of JSON file at line %lu",
                                    (unsigned long) curLine);
              }
              c = char(fileBuf[filePos]);
              filePos++;
...
bool JSONReader::parseJSONValue(JSONItem*& o, const std::string& s)
{
           ...
           s[0] == '+' || s[0] == '-' || s[0] == '.')
  {
    ...
    {
      throw NifSkopeError("invalid number syntax in JSON file at line %lu",
                          (unsigned long) curLine);
    }
